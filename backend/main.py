...
# Route to get an instrument by dbid or tag_id
@app.route('/nfc/instrument/<int:id>', methods=['GET'])
@app.route('/nfc/instrument_by_tag/<int:id>', methods=['GET'])
def get_instrument(id):
    instrument = Instrument.query.get(id) or Instrument.query.filter_by(tag_id=id).first()
    if not instrument:
        return jsonify({"error": "Instrument not found"}), 404
    return jsonify(instrument.to_dict()), 200

# Route to delete an instrument
...
# Route to handle image upload and retrieval
...
# Route to update instrument storage
...
def pair_instrument():
    ...
    try:
        ...
        if not data or 'tag_id' not in data or 'serial' not in data:
            return jsonify({"error": "Missing required fields: tag_id and serial"}), 400
        ...
        if existing_tag:
            return jsonify({"error": f"Tag ID {tag_id} is already paired with instrument: {existing_tag.name}"}), 409
        
        instrument = Instrument.query.filter_by(serial=serial).first()
        if not instrument:
            return jsonify({"error": f"No instrument found with serial number: {serial}"}), 404
        
        if instrument.tag_id is not None:
            return jsonify({"error": f"This instrument is already paired with tag ID: {instrument.tag_id}"}), 409
...
# Route to unpair a tag
@app.route('/nfc/unpair_tag/<int:tag_id>', methods=['DELETE'])
def unpair_tag(tag_id):
    instrument = Instrument.query.filter_by(tag_id=tag_id).first()
    if not instrument:
        return jsonify({"error": f"No instrument found paired with tag ID: {tag_id}"}), 404
    
    instrument_name = instrument.name
    instrument.tag_id = None
    db.session.commit()
    return jsonify({"message": f"Successfully unpaired tag ID {tag_id} from {instrument_name}"}), 200
